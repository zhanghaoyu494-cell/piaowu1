...
- SQLite connections enable `secure_delete`, the FTS5 index enables its dedicated secure-delete mode, and explicit memory or conversation deletion optimizes obsolete FTS segments, checkpoints and truncates WAL, then vacuums the database.
- Secret detection covers common API tokens, bearer tokens, private keys, AWS access-key IDs, and English or Chinese credential labels. Detection remains best effort rather than a complete data-loss-prevention system.
